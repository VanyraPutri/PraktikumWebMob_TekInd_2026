...
// Elemen untuk Tugas Proyek Mini - Kalkulator Listrik
...
const estimasiBiayaSpan = document.getElementById('estimasiBiaya');
...
// ========== LATIHAN 2: Hover Efek pada Suhu ==========

// Event saat mouse masuk (hover)
...
// ========== TUGAS PROYEK MINI: Kalkulator Tagihan Listrik ==========
function hitungTagihan() {
    let daya = parseFloat(inputDaya.value) || 0;
    let jam = parseFloat(inputJam.value) || 0;
    
    let kwh = (daya * jam) / 1000;
    let biaya = kwh * 1500;
    
    totalKwhSpan.innerHTML = kwh.toFixed(2);
    estimasiBiayaSpan.innerHTML = biaya.toLocaleString('id-ID');
}

// Event listener untuk kedua input (real-time)
inputDaya.addEventListener('input', hitungTagihan);
inputJam.addEventListener('input', hitungTagihan);